...
class CrawlOverview:
    def __init__(self, crawl_file=None):
        ...
        try:
            self.readydata['Protocol'].append(self.happy_data('http', self.crawl_data['Protocol']['HTTP'][0], [0, 1]))
        ...
        self.readydata['AMP'] = []
        try:
            self.readydata['AMP'].append(self.happy_data('Andere valideringsfouten:', self.crawl_data['AMP']['Other Validation Errors'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekende / ongeldige <meta viewport> tag:', self.crawl_data['AMP']['Missing/Invalid <meta viewport> Tag'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekend / ongeldig AMP-script:', self.crawl_data['AMP']['Missing/Invalid AMP Script'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekende AMP Canonical:', self.crawl_data['AMP']['Missing Canonical'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekende / ongeldige AMP-boilerplate', self.crawl_data['AMP']['Missing/Invalid AMP Boilerplate'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekende <html amp> Tag:', self.crawl_data['AMP']['Missing <html amp> Tag'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekende Canonical:', self.crawl_data['AMP']['Missing Canonical'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekende <head> tag:', self.crawl_data['AMP']['Missing <head> Tag'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Canonical kan niet worden bevestigd:', self.crawl_data['AMP']['Non-Confirming Canonical'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekende <body> -tag:', self.crawl_data['AMP']['Missing <body> Tag'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekend / ongeldig <meta charset> Tag:', self.crawl_data['AMP']['Missing/Invalid <meta charset> Tag'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Ontbrekend / ongeldig <! Doctype html> Tag:', self.crawl_data['AMP']['Missing/Invalid <!doctype html> Tag'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Niet 200 respons:', self.crawl_data['AMP']['Non-200 Response'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Niet-indexeerbaar:', self.crawl_data['AMP']['Non-Indexable'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Niet-indexeerbaar Canonical:', self.crawl_data['AMP']['Non-Indexable Canonical'][0], [0,1]))
            self.readydata['AMP'].append(self.happy_data('Bevat niet toegestane HTML:', self.crawl_data['AMP']['Contains Disallowed HTML'][0], [0,1]))
        except KeyError as e:
            print('Amp error : {}'.format(e))

        self.readydata['Response Codes'] = []
        try:
            self.readydata['Response Codes'].append(self.happy_data('Redirection (JavaScript):', self.crawl_data['Response Codes']['Redirection (JavaScript)'][0]))
            self.readydata['Response Codes'].append(self.happy_data('Doorverwijzing (Meta Refresh):', self.crawl_data['Response Codes']['Redirection (Meta Refresh)'][0]))
            ...
            self.readydata['Response Codes'].append(self.happy_data('Clientfout (4xx):', self.crawl_data['Response Codes']['Client Error (4xx)'][0], [0,1]))
            self.readydata['Response Codes'].append(self.happy_data('Serverfout (5xx):', self.crawl_data['Response Codes']['Server Error (5xx)'][0], [0,1]))
            self.readydata['Response Codes'].append(self.happy_data('Geblokkeerde bron:', self.crawl_data['Response Codes']['Blocked Resource'][0], [0,1]))
            self.readydata['Response Codes'].append(self.happy_data('Geen antwoord:', self.crawl_data['Response Codes']['No Response'][0], [0,1]))
            self.readydata['Response Codes'].append(self.happy_data('Geblokkeerd door Robots.txt:', self.crawl_data['Response Codes']['Blocked by Robots.txt'][0]))
        except KeyError as e:
            print('Responsecode error : {}'.format(e))

        self.readydata['Afbeeldingen'] = []
        try:
            self.readydata['Afbeeldingen'].append(self.happy_data('Alt-tekst Meer dan 100 tekens:', self.crawl_data['Images']['Alt Text Over 100 Characters'][0], [0,1]))
            self.readydata['Afbeeldingen'].append(self.happy_data('Ontbrekende Alt-tekst:', self.crawl_data['Images']['Missing Alt Text'][0], [0,1]))
            self.readydata['Afbeeldingen'].append(self.happy_data('Meer dan 100 KB:', self.crawl_data['Images']['Over 100 KB'][0], [0,1]))
        except KeyError as e:
            print('Afbeeldingen error : {}'.format(e))
            
        self.readydata['Paginatitels'] = []
        try:
            self.readydata['Paginatitels'].append(self.happy_data('Afwezig:', self.crawl_data['Page Titles']['Missing'][0], [0,1]))
            self.readydata['Paginatitels'].append(self.happy_data('Duplicaat:', self.crawl_data['Page Titles']['Duplicate'][0], [0,1]))
            self.readydata['Paginatitels'].append(self.happy_data('Meer dan 65 tekens:', self.crawl_data['Page Titles']['Over 65 Characters'][0], [0,1]))
            self.readydata['Paginatitels'].append(self.happy_data('Onder de 30 tekens:', self.crawl_data['Page Titles']['Below 30 Characters'][0], [0,1]))
        ...
        self.readydata['H1'] = []
        try:
            self.readydata['H1'].append(self.happy_data('Afwezig:', self.crawl_data['H1']['Missing'][0], [0, 1]))
            self.readydata['H1'].append(self.happy_data('Duplicaat:', self.crawl_data['H1']['Duplicate'][0], [0, 1]))
            self.readydata['H1'].append(self.happy_data('Meer dan 70 tekens:', self.crawl_data['H1']['Over 70 Characters'][0], [0, 1]))
            self.readydata['H1'].append(self.happy_data('Meerdere:', self.crawl_data['H1']['Multiple'][0], [0, 1]))
        except KeyError as e:
        ...
        try:
            ...
            self.readydata['Interne Urls'].append(self.happy_data('Flash', self.crawl_data['Internal']['Flash'][0]))
        ...
        try:
            ...
            self.readydata['Externe Urls'].append(self.happy_data('JavaScript', self.crawl_data['External']['JavaScript'][0]))
            self.readydata['Externe Urls'].append(self.happy_data('CSS', self.crawl_data['External']['CSS'][0]))
            ...
            self.readydata['Externe Urls'].append(self.happy_data('Flash', self.crawl_data['External']['Flash'][0]))
    ...
    @staticmethod
    def happy_data(name=None, grade=None, grades=None):
        if name is not grade is None:
            return ""

        data = []
        # blank line, because pycharm likes it
        data.append(name)
        data.append(grade)
        if grades is None:  #no grades? just returns
            data.append("")
            return data
