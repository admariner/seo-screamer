...
class AlgemeenOverzicht:
    def __init__(self, doc=None, config=None, co_readydata=None, co_table_headers=None):
        ...
        par.add_run("Achter iedere probleem melding vind u de oplossingsmoeilijkheid. Bij 1 ")
        run = par.add_run("")
        run.font.name = 'Font Awesome 5 Pro Light'
        par.add_run(" kunt u dit vrij eenvoudig zelf oplossen via de backend van uw site. Bij meer dan 3 ")
        run = par.add_run("")
        run.font.name = 'Font Awesome 5 Pro Light'
        ...
        for h in co_table_headers:
            ...
            if crawl_overview_conf[h]['table_header'] == 1:
                table = doc.add_table(rows=1, cols=crawl_overview_conf[h]['table_cols'], style="Grid Table 4 Accent 5")
                ...
                table = doc.add_table(rows=0, cols=crawl_overview_conf[h]['table_cols'])
            ...
            for r in co_readydata[h]:
                ...
                if crawl_overview_conf[h]['table_cols'] > 3:
                    try:
                        ...
                        print("We found an error in {} : {}".format(r[0], e))

            self.set_col_widths(table, crawl_overview_conf[h]['table_cols'])
    ...
    def set_col_widths(self, table, cells=4):
        ...
        for row in table.rows:
            for idx, width in enumerate(widths):
                ...
                table.cell(i, idx).vertical_alignment = WD_ALIGN_VERTICAL.CENTER
            i += 1

    def set_row_heights(selfs, table, height=0.7):
        for row in table.rows:
